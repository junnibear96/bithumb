...
# 출처1: "https://docs.bithumb.com/reference/%EC%A0%84%EC%B2%B4-%EA%B3%84%EC%A2%8C-%EC%A1%B0%ED%9A%8C"
...
## "current_price_rank23"에 'C:/Users/2101A00085/PycharmProjects/pythonProject/bithumb_trade_price/' 경로에 'bithumb_trade_price_rank_전일년월일_23'이라는 엑셀파일을 불러와 지정
current_price_rank23 = pd.read_excel(save_path + 'bithumb_trade_price_rank_' + (datetime.date.today() - datetime.timedelta(1)).strftime('%Y%m%d')
...
market_code_lookup = pd.DataFrame(requests.get(
    "https://api.bithumb.com/v1/market/all?isDetails=true", headers={"Accept": "application/json"}).json())
...
for i in range(len(market_code)):
    ...
    current_price = pd.DataFrame(requests.get("https://api.bithumb.com/v1/ticker?markets=" + market_code[i],
...
## "current_price_rank"를 'C:/Users/2101A00085/PycharmProjects/pythonProject/bithumb_trade_price/' 경로에 'bithumb_trade_price_rank_오늘년월일_8'이라는 엑셀파일로 저장
current_price_rank8.to_excel(save_path + 'bithumb_trade_price_rank_' + datetime.date.today().strftime('%Y%m%d') + '_8.xlsx', index=False)
# ==================================================
...
if volume_rising_stocks is not None:
    ...
    ## API로 업비트에서 내 계좌 조회
    my_exchange_account = pd.DataFrame(requests.get("https://api.bithumb.com/v1/accounts",
    ...
    if now_krw >= 7000:
        ...
        buy_market_order_data = pd.DataFrame.from_dict(pybithumb.Bithumb(A_key, S_key)
        ...
        ## executed_volume: 체결된 양, trades_count: 해당 주문에 걸린 체결 수
        ## API로 업비트에서 특정 주문 조회
        specific_order = pd.DataFrame.from_dict(pybithumb.Bithumb(A_key, S_key).get_order(order_uuid), orient='index').T
        ...
        while sec < (loop_time * 3600):
            ...
            buy_market_price_ascent = (pybithumb.get_current_price(market_code)
        ...
        # ==================================================
                    ...
                    market_code = volume_rising_stocks
                    ## 주문 수량
                    order_quantity = pybithumb.Bithumb(A_key, S_key).get_balance(market_code)   # 특정 종목 보유 수량
                    ...
                    sell_market_order_data = pd.DataFrame.from_dict(
                        pybithumb.Bithumb(A_key, S_key).sell_market_order(market_code, order_quantity), orient='index').T
            ...
            ## 수익률이 5% 이상인 순간 들여 쓴 코드 실행
                ...
                market_code = volume_rising_stocks
                ## 주문 수량
                order_quantity = pybithumb.Bithumb(A_key, S_key).get_balance(market_code)
